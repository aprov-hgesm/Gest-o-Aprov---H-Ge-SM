...
tabs_block = """            </div>
            ...
            {/* Modal Form Scrollable */}"""
replace_once(tabs_anchor, tabs_block, 'meal tabs')
...
replace_once(
    ...
    'event holiday visibility',
)

meal_visibility = [
    ('cafe', 'editingFocusMeal === \'cafe\' ? "bg-emerald-50/50 border-emerald-400 ring-2 ring-emerald-200" : "bg-white border-slate-200"'),
    ('colacao', 'editingFocusMeal === \'colacao\' ? "bg-emerald-50/50 border-emerald-400 ring-2 ring-emerald-200" : "bg-white border-slate-200"'),
    ('almoco', 'editingFocusMeal === \'almoco\' ? "bg-emerald-50/70 border-emerald-500 ring-2 ring-emerald-300" : "bg-emerald-50/40 border-emerald-200/80"'),
    ('jantar', 'editingFocusMeal === \'jantar\' ? "bg-emerald-50/50 border-emerald-400 ring-2 ring-emerald-200" : "bg-white border-slate-200"'),
    ('ceia', 'editingFocusMeal === \'ceia\' ? "bg-emerald-50/50 border-emerald-400 ring-2 ring-emerald-200" : "bg-white border-slate-200"'),
]
for meal, ternary in meal_visibility:
    replace_once(
        ternary + '\n              )}>',
        ternary + f',\n                editingFocusMeal !== \'all\' && editingFocusMeal !== \'{meal}\' && "hidden"\n              )}}>',
        f'{meal} visibility',
    )

protein_replacement = r'''                  {/* Sugestões secundárias ficam recolhidas para manter o foco no cadastro */}
...
'''
sub_once(
    r'                  \{/\* Preset chips for Proteínas \*/\}\n.*?(?=                </div>\n\n                \{/\* Tipo da Carne)',
    protein_replacement,
    'protein suggestion collapse',
)

replace_once(
    'className="space-y-3 p-3.5 rounded-xl bg-amber-50/90 border-2 border-amber-300 shadow-2xs"',
    'className="space-y-3 p-4 rounded-xl bg-amber-50/60 border border-amber-300"',
    'meat panel style',
)
replace_once(
...
replace_once(
    ...
    'meat panel description',
)
replace_once('Detalhamento Interno', 'Operacional', 'meat panel badge')

meat_shortcuts = r'''                  {/* Atalhos secundários de corte e quantidade */}
...
'''
sub_once(
    r'                  \{/\* Atalhos de Kg rápidos \*/\}\n.*?(?=                  \{/\* Caixa de Cálculo do Saque)',
    meat_shortcuts,
    'meat shortcuts collapse',
)
...
weekly_block = """        </div>
      ...
      {/* ------------------------------------------------------------------- */}
      {/* VIEW 1: STRUCTURED EDITOR BY DAY                                    */}"""
replace_once(weekly_anchor, weekly_block, 'weekly protein overview')
...
replace_once(
    'className="block text-black font-black text-[11px]"',
    'className="block text-black font-black text-[13px]"',
    'saque date emphasis',
)
replace_once(
    'className="text-black font-black text-[11px] block"',
    'className="text-black font-black text-[13px] block"',
    'saque cut emphasis',
)
replace_once(
    'className="border border-black px-2 py-1.5 text-center font-black text-black text-[11px]"',
    'className="border border-black px-2 py-1.5 text-center font-black text-black text-[13px]"',
    'saque qty emphasis',
)

path.write_text(text, encoding='utf-8')
print('Cardapio UI refactor applied successfully.')
